 ...
 * (`PATCH`/`POST`/`DELETE /{type}/{id}/relationships/{rel}`). The {@see $parent} is
 * the loaded, **mutable** owner; {@see $relation}, {@see $linkage} and {@see $mode}
 * describe the requested change. A listener that throws a
 * {@see \haddowg\JsonApi\Exception\JsonApiExceptionInterface} aborts before the
 * persister applies the change. Routed to
 * {@see \haddowg\JsonApiLaravel\Hook\ResourceLifecycleHooksInterface::beforeRelationshipMutate()}.
 */
