 ...
 * the live registry (its resource descriptor, resource/relations, paginator, the
 * authorization overrides, and resolved tags) into a plain immutable carrier the core
 * projector reads.
 *
 * It tolerates a **standalone** type (no resource): `hasFields()` is then `false`,
 * `fields()`/`relations()`/`filters()`/`sorts()` are empty, and the projector emits a
 * permissive resource-object schema. Everything else — uriType, operations, tags,
