 ...
 * docs.
 *
 * It is a plain immutable carrier built from `jsonapi.openapi.*` config (and the
 * per-server base URI) and injected into the {@see MetadataSource}. A `null`/empty
 * member means the document omits it; defaults (a fallback title/version, the
 * per-server base URI as the single advertised server) are the metadata source's job,
 * applied when this carrier supplies none.
 *
 ...
 * *references* but config did not *define* — so this carries only the config-declared
 * definitions, in config order.
 */
