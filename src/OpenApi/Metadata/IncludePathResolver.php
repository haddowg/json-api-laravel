...
/**
 * Derives the dotted `?include` paths a type advertises
 * ({@see \haddowg\JsonApi\OpenApi\Metadata\TypeMetadataInterface::includablePaths()}),
 * by walking the relation graph from a root type — neither core nor the package stores
 * the resolved path set, so the metadata source derives it here.
 *
 ...
 *    ({@see Server::maxIncludeDepth()}); a `null` cap is unlimited (but cycle detection
 *    still terminates the walk);
 *  - **allow-list**: when the root resource declares a `getAllowedIncludePaths()`
 *    whitelist, only paths it permits (a path is permitted when it is a prefix of, or
 *    equal to, an allowed path) are advertised.
 *
 * Cycle safety: a path that revisits a type already on the current branch is not
 * descended again, independent of the depth cap.
 *
 * @internal
 */
final class IncludePathResolver
{
    ...
    /**
     * The includable dotted paths for `$type` on `$server`, as the primary type of its
     * own collection / read endpoints. Empty when the type has no includable relations
     * (or no resource).
     *
     * @return list<string>
     */
     ...
     * related resource is the primary data there, so the walk roots on it with its own
     * depth cap / allow-list. For a polymorphic relation (several related types sharing
     * no include vocabulary) the result is empty.
     *
     * @return list<string>
     */
    ...
    private function walk(Server $server, string $type, string $prefix, array $branch, ?int $maxDepth, ?array $allowList): array
    {
        ...
        foreach ($this->types->relationsFor($server, $type) as $relation) {
            ...
            // The related type(s) must be serializable on THIS server: a relation may
            // point at a type registered only on another server, which renders links-only
            // here and hydrates nothing on `?include` — advertising the path would emit an
            // include token the server can never fulfil.
     ...
     * `$server`. A monomorphic relation checks its single related type; a polymorphic
     * one requires **all** member types.
     */
    ...
    /**
     * Whether `$path` is permitted by the root resource's allow-list: always true when
     * there is no list; otherwise true when `$path` equals, or is a dotted prefix of,
     * any allowed path.
     *
     * @param list<string>|null $allowList
     */
