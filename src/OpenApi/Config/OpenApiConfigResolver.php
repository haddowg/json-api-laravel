...
/**
 * Resolves the `jsonapi.openapi.*` configuration array into a typed
 * {@see OpenApiConfig} — the runtime translation of the config tree into the OAS
 * value-object model.
 *
 ...
 * default title, which is left null so the {@see \haddowg\JsonApiLaravel\OpenApi\Metadata\MetadataSource}
...
{
    /**
     * @param array<string, mixed> $openapi the `jsonapi.openapi` config array
     * @param list<string>         $servers the declared server names
     */
    public function resolve(array $openapi, array $servers): OpenApiConfig
    {
        ...
        return new OpenApiConfig(
            ...
            combined: ($openapi['multi_server'] ?? 'separate') === 'combined',
            enumDescriptionMode: EnumDescriptionMode::tryFrom(\is_string($openapi['enum_value_descriptions'] ?? null) ? $openapi['enum_value_descriptions'] : 'both') ?? EnumDescriptionMode::Both,
    ...
    private function license(array $info): ?License
    {
        ...
        // are given (the more precise OAS 3.1 form).
    ...
    /**
     * @param array<string, mixed>          $openapi
     * @param array<string, SecurityScheme> $schemes the resolved security schemes — a default requirement naming a scheme not present here is dropped (it would otherwise be a dangling reference)
     *
