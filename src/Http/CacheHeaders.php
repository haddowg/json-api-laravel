 ...
 * `jsonapi.defaults.cache_headers` config key (PLAN decision 12, bundle ADR 0054).
 * A pure, immutable value object of scalars.
 *
 * It maps onto a response via {@see applyTo()}: the RFC-7234 `Cache-Control`
 * directives (`max-age`, `s-maxage`, `public`/`private`, `no-cache`,
 * `must-revalidate`) plus the `Vary` header. Laravel's `Illuminate\Http\Response`
 * extends the Symfony {@see Response}, so `setCache()`/`setVary()` are available. A
 * {@see CacheHeaders} is applied **only** to a successful `GET` data response —
 * never to a write or an error (the
 * {@see \haddowg\JsonApiLaravel\Http\ResponseHeadersMiddleware} enforces that).
 *
 * The directives are intentionally all-optional and orthogonal: a `CacheHeaders`
 * with everything `null`/`false` is {@see isEmpty()} and applies nothing.
 */
final readonly class CacheHeaders
{
    ...
    /**
     * Rebuilds a {@see CacheHeaders} from the scalar shape discovery + the global
     * `cache_headers` config key use: a map of `max_age`, `s_maxage`,
     * `public`/`private`, `no_cache`, `must_revalidate`, `vary`.
     *
     * `public: true` and `private: true` are mutually exclusive; `private` wins (it
     * is the more conservative default for an authenticated API).
     *
     * @param array{
     *     max_age?: int|null,
     *     s_maxage?: int|null,
     ...
     * directive this object leaves unset inherits the default's value, so a resource
     * overrides only what it declares (and a per-operation override layers over the
     * resource-level one the same way).
     */
    ...
    /**
     * Writes the declared directives onto `$response`. `Cache-Control` is set through
     * {@see Response::setCache()} (so `max-age`/`s-maxage`/`public`/`private`/
     * `no-cache`/`must-revalidate` compose correctly) and each `Vary` header name is
     * added (`replace: false`, so an app-set `Vary` is appended to, not clobbered).
     */
