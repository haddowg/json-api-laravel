...
namespace haddowg\JsonApiLaravel\DataProvider\Keyset;
 ...
 * both providers so the Eloquent forced `ORDER BY`/keyset `WHERE` and the
 ...
 * plain {@see \haddowg\JsonApiLaravel\DataProvider\CriteriaApplier} would — a
 ...
 * `id >/< :v` tiebreak in the keyset WHERE the provider builds from these columns.
 */
final class KeysetResolver
{
     ...
     * {@see \haddowg\JsonApiLaravel\DataProvider\CriteriaApplier} exactly so the
