...
/**
 * The in-memory keyset execution — the **ground truth** the Eloquent push-down
...
final class InMemoryKeyset
{
     ...
     * twin of the Eloquent forced `ORDER BY` — per column: non-null sorts before
     ...
     * Eloquent `ORDER BY`); two non-nulls compare by value, flipped for desc.
     */
    private function compareValues(mixed $a, mixed $b, bool $descending): int
    {
        ...
        // Eloquent `(c IS NULL) DESC` leading term.
        if ($aNull || $bNull) {
            if ($aNull && $bNull) {
                return 0;
            }
