 ...
 * a thin provider — one that serves a flat collection with no relationships, or delegates
 * the rest to another provider — extends this and writes only the three abstracts, instead
 * of stubbing all nine methods.
 *
 * **A type whose relations are actually served must override the relevant seam.** In
 * particular a type whose **to-one relations are filtered** must override
 * {@see relatedToOneMatches()} *and* {@see relatedToOneMatchesBatch()} so an excluded
 * target is nulled — the defaults here deliberately match **every** target (they never
 * null), since a base provider that does not know how to filter a to-one must not
 * silently drop one.
 *
...
abstract class AbstractDataProvider implements DataProviderInterface
{
     ...
     * result and the include preloader loads this relation lazily instead. Override to
     * batch a to-many include without N+1.
     */
     ...
     * *matching* to-one, so the absent-capability value is `true`. A type whose to-one
     ...
     * conventional `id` member — the same value the default serializer's `getId()` reports
     * for the common case. A type with a non-`id` id member, an id encoder, or an
     * actually-filtered to-one MUST override this (and {@see relatedToOneMatches()}) so the
     * map keys — and the filter probe — match the serializer exactly.
     *
     ...
     * relationship member validates in the create (new-row) context. Override to fold a
     * stored pivot row under an incoming member's meta on an update.
     *
