 ...
 * only to project the action's success response in the generated OpenAPI document (the
 * runtime response is whatever the handler returns). Mirrors core's
 * {@see \haddowg\JsonApi\OpenApi\Metadata\ActionOutputMode} by case name.
 *
 * - {@see Document} (default): a JSON:API document whose `data` is the action's
 *   `outputType` resource (a `200`; the handler returns a
 *   {@see \haddowg\JsonApi\Response\DataResponse}).
 * - {@see Meta}: a JSON:API document whose primary content is its top-level `meta`, no
 *   `data` (a `200` with the shared meta-document schema; the handler returns a
