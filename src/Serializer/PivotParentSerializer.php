 ...
 * (`GET /{type}/{id}/relationships/{rel}`) — and the `200` linkage echo of a pivot
 * relationship mutation — render a `belongsToMany` pivot relation's per-member pivot values
 * as identifier `meta.pivot`.
 *
 * That endpoint renders the parent through the parent's serializer with the requested
 * relationship name, so the linkage is built by the parent serializer's `getRelationships()`
 * resolving the related type's serializer. This decorator delegates everything to the inner
 * parent serializer (including every optional serializer-render interface, via
 * {@see AbstractPivotParentSerializer}) EXCEPT that ONE relationship: for it, the
 * relationship value object is rebuilt via the relation's public
 * {@see RelationInterface::buildRelationship()} with a {@see PivotSubstitutingResolver} that
 * returns the {@see PivotMetaSerializer} for the related type — so the linkage identifiers
 * carry their pivot meta. Every other relationship keeps the inner serializer's callable
 * untouched.
 *
 * The pivot map covers the WHOLE association (the relationship endpoint renders all linkage
 * off the parent, not a windowed page), so the provider supplies a full map for this render.
 */
