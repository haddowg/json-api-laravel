 ...
 * ({@see ResourceLifecycleHooksInterface}) receives: the live JSON:API request, the
 * server name the operation dispatched on, the resource type, and — for the
 * relationship-mutation pair — the {@see RelationInterface} being mutated, the
 * parsed linkage, and the {@see Mode} of the mutation.
 *
 * The built-in {@see \haddowg\JsonApiLaravel\EventListener\ResourceHookSubscriber}
 * assembles it from the dispatched event before routing to the resource's hook
 * method, so a resource's hooks read the request/relation context without depending
 * on the package's event classes.
 */
