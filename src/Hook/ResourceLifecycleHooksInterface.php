...
namespace haddowg\JsonApiLaravel\Hook;
 ...
 * interface (and `use`-ing {@see ResourceLifecycleHooksTrait} for no-op defaults),
 * then overriding only the hooks it needs.
 *
 * The hooks are routed by the built-in
 * {@see \haddowg\JsonApiLaravel\EventListener\ResourceHookSubscriber}, which listens
 * to every lifecycle event the {@see \haddowg\JsonApiLaravel\Operation\CrudOperationHandler}
 ...
 * are sugar over the events: a single dispatch point, no per-type listener
 * registration.
 *
 * The interface lives in the **package**, not on core's `AbstractResource`, so core
 * stays free of any dependency on the package's event/context types: a resource
 * opts in here without core knowing the hooks exist. An application has **two**
 * equivalent ways to hook the lifecycle: register its own listener on an event class
 * (a cross-cutting concern via `Event::listen`), or implement this interface on a
 * resource (a per-type concern) — both run from the one dispatch point.
 *
 * Semantics:
 ...
 *    {@see \haddowg\JsonApiLaravel\Exception\JsonApiExceptionRenderer} renders it (a
 *    `403` guard, a `422` imperative-validation failure, a `409` conflict). A
 *    mutation a before hook makes to the entity is persisted (it runs before the
 *    persister flush);
 ...
 *    Under an active Atomic Operations batch the after hooks fire post-commit and
 *    their replacement is inert (the aggregate batch result wins).
 *
 * The aggregate `beforeSave`/`afterSave` pair wraps **both** create and update —
 * `$creating` distinguishes which — so a concern that applies to every write (audit,
 * timestamps) lives in one place.
 *
 * **The division from Eloquent model events (PLAN decisions 10/12).** These hooks
 * (and the events behind them) are the *JSON:API* lifecycle: they fire only on an
 * API-driven write, carry the operation context (request, server, relation/linkage),
 * and — under an Atomic Operations batch — defer their After* half to post-commit.
 * Eloquent's own model events (`creating`/`saved`/…) are a **separate, untouched**
 * layer: the persister calls `$model->save()`, so they fire on *every* write path
 * (a seeder, a console command, a batch sub-op mid-transaction), a pure persistence
 * concern. Use a model event for a storage invariant that must hold no matter who
 * writes; use these hooks for an API-request concern (authorization, request-aware
 * shaping, response replacement).
 */
interface ResourceLifecycleHooksInterface
{
    /**
     * Before a create or an update persists, with `$creating` distinguishing the
     * two; the entity is mutable and a throw aborts. Fires before the more specific
     * {@see beforeCreate()}/{@see beforeUpdate()}.
     */
    public function beforeSave(object $entity, bool $creating, HookContext $context): void;

    /**
     * After a create or an update commits, with `$creating` distinguishing the two;
     * may replace the response. Fires after the more specific
    ...
    /**
     * Before an update persists; the entity is mutable, `$original` is a pre-change
     * snapshot of it, and a throw aborts.
     */
