...
/**
 * The in-memory write witness: it persists through the same {@see InMemoryStore} an
 * {@see \haddowg\JsonApiLaravel\DataProvider\InMemoryDataProvider} reads from, so a created,
 * updated, or deleted resource is immediately visible to reads — the write twin of that
 * provider, mirroring how the Eloquent pair shares one connection.
 *
 * One instance answers for a single `$type`. A `$factory` produces the blank instance the
 * handler hydrates on create (the store holds no class metadata of its own).
 *
 * For relationship mutations the store holds the related **objects** (not raw ids): so
 * {@see mutateRelationship()} resolves a linkage id back to the stored object through the
 * optional `$relatedResolver` — a `(type, id) → ?object` lookup wired across the related
 * types' stores — and sets the parent's association property to that object (or list of
 * objects). A persister with no resolver supports only whole-resource writes.
 */
final class InMemoryDataPersister implements DataPersisterInterface, TransactionalDataPersisterInterface
{
    ...
    /**
     * Snapshots the shared store, so the subsequent in-memory writes can be discarded by
     * {@see rollback()} or kept by {@see commit()} — the in-memory analogue of opening a
    ...
    /**
     * Keeps every write made since {@see beginTransaction()} (discarding the snapshot), the
     * in-memory analogue of committing.
     */
    ...
    private function applyToMany(
        ...
        // Mode::Add — append the resolved members, deduplicating on id so add is idempotent
    ...
    /**
     * The JSON:API id of a stored related object — read off its public `id` member, the same
     * shape the seed objects use.
     */
