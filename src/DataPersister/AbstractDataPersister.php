...
/**
 * The convenience on-ramp for a custom {@see DataPersisterInterface}: an abstract base that
 * leaves the irreducible write core — {@see supports()}, {@see instantiate()}, {@see create()},
 * {@see update()}, {@see delete()} — abstract, and supplies a default {@see mutateRelationship()}
 * a persister that never serves the `…/relationships/{rel}` write endpoints (nor embeds
 * relationships in whole-resource writes) does not have to hand-stub.
 *
 * The handler calls {@see mutateRelationship()} *unconditionally* for a relationship write, so
 * the default **throws** a clear {@see \LogicException} rather than silently succeeding: a
 * persister that only does whole-resource create/update/delete simply never triggers it; one
 * that exposes a writable relationship must override it (and resolve the linkage ids to the
 * related objects/references its storage uses).
 */
abstract class AbstractDataPersister implements DataPersisterInterface
{
     ...
     * resource-identifier ids to the related objects/references your storage uses and mutate
     * `$relation`'s association on `$entity` under `$mode`.
     */
