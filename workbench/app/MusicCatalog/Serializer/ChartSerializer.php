...
/**
 * The standalone serialize-plus-fetch witness (capability composition, PLAN decision 3,
 * bundle ADR 0024): a hand-written serializer for the `charts` type registered with
 * {@see AsJsonApiSerializer} — there is **no** `charts` Resource, **no** Eloquent model,
 * and **no** hydrator. The type is read-only, opened to exactly two endpoints by the
 * `operations` allow-list (`GET /charts` and `GET /charts/{id}`); without it a standalone
 * serializer defaults to **none** (serialize-only) — the default-operations asymmetry
 * against an `AbstractResource` (which defaults to all five). Its data comes from the
 * custom {@see \Workbench\App\MusicCatalog\Provider\ChartProvider}.
 *
 * Re-themed verbatim from the bundle example's `ChartSerializer`. It implements
 * {@see UriTypeAwareInterface} so its URI path segment resolves explicitly to `charts`.
 * The seven {@see \haddowg\JsonApi\Serializer\SerializerInterface} methods are
 * hand-written — `getAttributes()` returns a map of callables, each invoked only for the
 * members that survive sparse-fieldset filtering.
 */
