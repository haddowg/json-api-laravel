...
/**
 * The composite-attribute witness resource, shared by both providers — the Laravel twin
 * of the Symfony bundle's `BaseCompositeWidgetResource`: an {@see Obj} `address` (typed
 * object in one value, per-child constraints), a discriminated {@see OneOf} `block`,
 * and a free-form {@see ArrayHash} `contact` carrying a {@see Shape} composite-schema
 * constraint (a discriminated `oneOf` of an email and a phone shape, raw member
 * schemas). The Obj/OneOf children exercise the bridge's dotted-key cascade
